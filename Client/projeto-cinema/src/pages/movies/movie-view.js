...
export default function MovieView({ history }) {
    ...
    async function removeMovie(movie) {
        try {
            const id = movie.Id;
            const response = await api.delete(`api/movie`, { data: { id } }, {
                headers: {
                    token: sessionStorage.getItem('token'),
            ...
            const isDelete = JSON.parse(response.data);
                        ...
                        {movies.map(movie => (
                            <li key={movie.Id}>
                                ...
                                <footer>
                                    ...
                                    <button className="buy" onClick={() => handleToEdit(movie.Id)}>Editar</button>
